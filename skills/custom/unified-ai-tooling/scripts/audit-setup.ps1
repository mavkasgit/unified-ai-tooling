...
function Resolve-RepoRoot {
    if ($RepoRoot -and (Test-Path (Join-Path $RepoRoot 'manifest.json'))) { return $RepoRoot }
    $cfg = Join-Path $env:USERPROFILE '.config\ai\tooling-repo.json'
    if (Test-Path $cfg) {
        $root = (Get-Content $cfg -Raw | ConvertFrom-Json).repoRoot
        if ($root -and (Test-Path (Join-Path $root 'manifest.json'))) { return $root }
    }
    foreach ($c in @(
            (Join-Path $env:USERPROFILE 'VibeCoding\unified-mcp'),
            (Join-Path $env:USERPROFILE 'VibeCoding\unified-ai-tooling')
        )) {
        if (Test-Path (Join-Path $c 'manifest.json')) { return $c }
    }
    throw 'RepoRoot not found. Run install.ps1 from unified-ai-tooling repo or pass -RepoRoot'
}
